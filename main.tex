...
This allows the problem of tie-breaking to be reduced to satisficing search within a plateau (\refsec{sec:astar-as-sat}), opening a wide variety of future work.
...
it does not compromise the admissibility of \astar as long as it is used only for tie-breaking.

{\bf [This paper presents work  from Sections 7-8 from a recent journal paper \cite{asai2017tie}. This work  has not been previously presented in any conference or workshop.]}
...
$f$-value and the $h$-value.  Each node in a $h$-plateau is stored into
a bucket of the corresponding depth $d$, and the expansion happens on a
node in a bucket that is selected at random, or in a round-robin manner
(deterministic version). Such a configuration of \astar is denoted as $[f,h,\depth,*]$.
...
This means that the search within the same $f$ plateau can be an arbitrary satisficing search without any cost minimization requirement (as opposed to the ``satisficing'' track setting in IPC which also seeks to minimize the plan cost with anytime algorithms). For example,
...
% 
In the following sections, we show that this framework  (1) provides a better understanding of depth-diversification (\refsec{sec:depth-vs-types}) and (2) allows us to improve the performance of \astar on Zerocost domains (\refsec{sec:distance-to-go}).

\subsection{Depth Diversification and Satisficing Search}
...
\section{Tie-Breaking with Distance-to-Go Estimates}
...
we used $\hat{h}$, the distance-to-go version of the original heuristic function $h$, as well as a distance-to-go variation of FF
heuristic ($\hat{h}^{\ff}$).
...
Detailed per-domain results are shown in \reftbl{tbl:dtg-lmcut-zero}.
...
\def\rb#1{\rotatebox{-90}{\hspace{-1em}#1\;}} 
...
\begin{table*}[htbp]
 ...
 \caption{
 Coverage results with \lmcut (top) and \mands (bottom) for computing $f$, and various tie-breaking strategies, on 620 Zerocost instances and 1104 IPC instances. We only show the domains when the difference between the maximum and the minimum coverage exceeds 2, and highlight the best results.
 }
...
When the primary heuristic is  \mands, $[f^\mands,h^\mands,\lifo]$ performs slightly better than  other default tie-breaking strategies; It also outperforms the depth-based variants.
...
% 
This differs from our work on cost-optimal search because 
...
\begin{comment} % relationship to A*-as-sequence-of-sat weak
...
We introduced a new interpretation of cost-optimal \astar search as a series of satisficing
...
This perspective led to a novel approach for effective tie-breaking in Zerocost domains, the use of
